...
import { d as db, a as commentVotes, c as comments } from "../../../../../../chunks/db.js";
...
const POST = async ({ params, request, locals }) => {
  ...
  const existing = await db.select().from(commentVotes).where(and(eq(commentVotes.userId, locals.user.id), eq(commentVotes.commentId, commentId))).limit(1);
  if (existing.length) {
  ...
  const [scoreResult] = await db.select({ score: sql`COALESCE(SUM(${commentVotes.value}), 0)` }).from(commentVotes).where(eq(commentVotes.commentId, commentId));
