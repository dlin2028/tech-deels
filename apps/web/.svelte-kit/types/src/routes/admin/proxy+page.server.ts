...
export const actions = {
  banUser: async ({ request }: import('./$types').RequestEvent) => {
  ...
  unbanUser: async ({ request }: import('./$types').RequestEvent) => {
    const data = await request.formData();
    const userId = String(data.get("userId") ?? "");
    if (!userId) return fail(400, { error: "User ID required" });
  ...
  deleteDeal: async ({ request }: import('./$types').RequestEvent) => {
    const data = await request.formData();
  ...
  resolveFlag: async ({ request }: import('./$types').RequestEvent) => {
    const data = await request.formData();
  ...
  addCategory: async ({ request }: import('./$types').RequestEvent) => {
    const data = await request.formData();
